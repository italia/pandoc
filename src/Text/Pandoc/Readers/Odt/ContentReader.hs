...
read_code :: BlockMatcher
read_code = matchingElement NsText "p" $ read_code_block

read_code_block :: OdtReaderSafe (Inlines, XML.Content) Block
read_code_block = liftA codeBlock $ read_code_text

read_code_text :: OdtReaderSafe (Inlines, XML.Content) String
read_code_text = proc x -> do
  fst ^&&& read_code_text' >>% recover -<< x
  where
    -- fallible version
    read_code_text' :: OdtReader (Inlines, XML.Content) String
    read_code_text' = proc x -> do
      fStyle <- readStyleByName -< x
      case fStyle of
        -- `Preformatted_20_Text` is available in Libreoffice 5 for
        -- styling paragraphs
        Right ("Preformatted_20_Text", _) ->
          ((second (arr extractText) >>^ spreadChoice) >>?% mappend) -< x
        _    -> arr $ const failEmpty -< ()
    --
    extractText :: XML.Content -> Fallible String
    extractText (XML.Text cData) = succeedWith (XML.cdData cData)
    extractText         _        = failEmpty
...
read_header       = matchingElement NsText "h" $  proc blocks -> do
